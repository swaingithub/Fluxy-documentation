...
You can use `fluxComputed()` to filter or transform collections reactively.

```dart
final highPriorityTasks = fluxComputed(() {
  return tasks.value.where((t) => t.isHighPriority).toList();
});
```
