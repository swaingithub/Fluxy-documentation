...
class TaskRepository extends FluxRepository {
  final tasks = flux<List<Task>>([], key: 'offline_tasks', persist: true);
  ...
  void onInit() {
    ...
    fluxEffect(() {
      if (isOnline.value && tasks.value.isNotEmpty) {
        _uploadPendingTasks();
      }
    });
  }
  ...
  Future<void> _uploadPendingTasks() async {
    Fx.toast.info("Connection Restored. Syncing ${tasks.value.length} items...");
    
    // Copy list to avoid concurrent modification issues during iteration
    final pending = List<Task>.from(tasks.value);
    for (var task in pending) {
      final success = await Fx.http.post('/tasks', body: task.toJson());
      if (success) {
        tasks.value = List<Task>.from(tasks.value)..remove(task);
      }
    }
  }
}
