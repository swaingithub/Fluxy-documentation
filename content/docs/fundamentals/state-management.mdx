...
class ChatRepository extends FluxyRepository {
  ...
  late final messages = flux<List<String>>([], 
    key: userScope(auth.userId, 'chat_history'),
    persist: true
  );
...
// Any flux with `key` and `persist: true` is automatically hydrated
final theme = flux("dark", key: "app_theme", persist: true);
final userPrefs = flux<Map<String, dynamic>>({}, key: "user_preferences", persist: true);
```
...
class AuthRepository extends FluxRepository {
  late final session = flux<dynamic>(null, key: 'user_session', persist: true);
