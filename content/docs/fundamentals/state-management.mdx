...
| Async State | `fluxAsync<T>` | Built-in async state handling |
...
**Note**: The `flux` function creates reactive signals that automatically update UI when values change. This is the core of Fluxy's reactivity system.
...
### 2. `fluxSelector` (Targeted Rebuilds)
...
Scoped state that automatically disposes with the widget:
