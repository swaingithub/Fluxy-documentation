...
Fx.col(
  ...
  gap: 8,
  children: [
    Fx.button("Login").fullWidth().render(),
    Fx.button("Register").outline().fullWidth().render(),
  ],
)
```
...
Fx.stack(
  ...
  fit: StackFit.loose,
  children: [
    Fx.image("background.jpg").cover().render(),
    Fx.text("Overlay Text").white.bold().render(),
  ],
)
```
