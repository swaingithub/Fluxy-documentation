...
In v0.1.1, the `fluxy` CLI and the `fluxy` Flutter package are synchronized. Ensure your CLI is up to date to support the latest OTA features.

```bash
dart pub global activate fluxy # Sync to v0.1.1
```
...
FxRemoteView(
  ...
  loading: Fx.text("Syncing...").render(),
)
```
