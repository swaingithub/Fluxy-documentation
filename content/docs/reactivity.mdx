...
```dart
Fx.text(() => "Current: ${count.value}").render();
```

## Two-Way Binding

Fluxy v0.1.1 provides native two-way binding for interactive components. Instead of manually handling `onChanged` and `controller.text`, you can bind a signal directly.
