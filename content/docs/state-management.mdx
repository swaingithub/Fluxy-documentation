...
For organized business logic, extend `FluxyController`. This provides built-in lifecycle management.
...
class UserController extends FluxyController {
  ...
  @override
  void onDispose() {
    // Called when Fluxy.delete<UserController>() is called
...
### Manual Cleanup
If you need to remove a dependency from memory manually:

```dart
Fluxy.delete<UserController>();
```
