...
## Proxy-based DSL (New in 0.1.2)

Fluxy 0.1.2 introduces a more expressive **Proxy-based DSL** that allows you to access style properties as fields rather than methods for common tokens. This results in even cleaner, more readable code.

```dart
Fx.text("Hello Fluxy")
  .weight.bold      // Proxy syntax for weight
...
### Fluent Flex Modifiers (New in 0.1.3)
...
- `.justifyAround()`
- `.justifyEvenly()`
- `.justify(MainAxisAlignment.value)` (Generic)
...
- `.itemsStretch()`
- `.itemsBaseline()`
- `.items(CrossAxisAlignment.value)` (Generic)
